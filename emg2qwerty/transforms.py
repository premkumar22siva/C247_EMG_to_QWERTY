...
# @dataclass
# class RandomCrop:
#     """Randomly crops the input tensor along the time dimension.

#     Args:
#         crop_size (int): Number of timesteps to retain.
#         time_dim (int): The dimension representing time (default: 0).
#     """
...
#     def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
#         if tensor.shape[self.time_dim] <= self.crop_size:
#             return tensor  # No cropping if input is too short
        
#         start = np.random.randint(0, tensor.shape[self.time_dim] - self.crop_size)
#         return tensor.narrow(self.time_dim, start, self.crop_size)

# @dataclass
# class RandomChannelDropout:
#     """Randomly drops out a subset of electrode channels during training.

#     Args:
#         drop_prob (float): Probability of dropping each channel independently (default: 0.2).
#         channel_dim (int): Dimension corresponding to electrode channels (default: -2).
#     """

#     drop_prob: float = 0.2
#     channel_dim: int = -2  # Assuming (T, N, Bands, Channels, Freq)

#     def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
#         # Create dropout mask (1 = keep, 0 = drop)
#         dropout_mask = (torch.rand(tensor.shape[self.channel_dim], device=tensor.device) > self.drop_prob).float()

#         # Expand mask dimensions to match tensor and apply dropout
#         return tensor * dropout_mask.view([1] * (tensor.ndim - 1) + [-1])
